  ...
  try {
    ...
    if (process.env.NODE_ENV === "production") {
      await sendEmail({
        ...
        text: `Please verify the following user: ${req.body.displayName} (${req.body.email}). ${process.env.BASE_URL}/approve/${token}`,
        html: `Please verify the following user:<br/>${req.body.displayName} (${req.body.email}). ${process.env.BASE_URL}/approve/${token}`,
      });
    } else {
