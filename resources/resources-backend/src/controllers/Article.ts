...
const getArticle = (req: Request, res: Response, next: NextFunction) => {
  ...
  return Article.findById(articleId)
    .then((article) => (article ? res.status(200).json({article}) : res.status(404).json({message: 'Article not found'})))
    .catch((error) => res.status(500).json({ error }));
...
const deleteArticle = (req: Request, res: Response, next: NextFunction) => {
  ...
  return Article.findByIdAndDelete(articleId)
    .then((article) => (article ? res.status(201).json({message: 'Article Deleted'}) : res.status(404).json({message: 'Article not found'})))
    .catch((error) => res.status(500).json({ error }));
    
}


export default { createArticle, getArticle, getAll, updateArticle, deleteArticle };
